...
# Добавляем критические HAL файлы, которые могут отсутствовать в subdir.mk
# (например, UART, если CubeMX не добавил их в link)
drivers_dir = os.path.join(PROJECT_DIR, "Drivers")
if os.path.isdir(drivers_dir):
    for entry in os.listdir(drivers_dir):
        if "_HAL_Driver" in entry:
            hal_src = os.path.join(drivers_dir, entry, "Src")
            if os.path.isdir(hal_src):
                # Список критических HAL модулей, которые должны быть в проекте
                critical_hal_patterns = [
                ...
                for pattern in critical_hal_patterns:
                    files = glob.glob(os.path.join(hal_src, pattern))
                    for f in files:
                        if f not in all_sources:
                            all_sources.append(f)
                            print(f"[OK] Added missing HAL file: {os.path.basename(f)}")
