        ...
        for entry in os.listdir(drivers_dir):
            if "_HAL_Driver" in entry:
                ...
                if os.path.isdir(hal_src):
                    ...
                    # Добавляем ВСЕ файлы HAL, даже если их нет в Makefile
                    # Это нужно для случаев, когда CubeMX не скомпилировал некоторые модули
                    extra_hal = glob.glob(os.path.join(hal_src, "stm32f4xx_hal_spi*.c"))
                    for f in extra_hal:
